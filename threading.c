...
int32_t t_create(fptr foo, int32_t arg1, int32_t arg2)
{
    ...
    int next_cont = find_next_free(current_context_idx);
    if (next_cont == -1) return 1;
...
int32_t t_yield()
{
    ...
    int next_cont = find_next_valid(current_context_idx);

    if (next_cont == current_context_idx || next_cont == -1) {
        return -1;
    }
...
void t_finish()
{
    ...
    int nxt = find_next_valid(current_context_idx);
    if(nxt == -1) exit(0); 

    swapcontext(&contexts[current_context_idx].context, &contexts[nxt].context);
}
